...
<script>
import { getEmployeesJobs, updateEmployeesJobs } from '@/api/employees.js'
...
export default {
  ...
  methods: {
    ...
    async loadEmployeesJobs() {
      const res = await getEmployeesJobs(this.Id)
      this.formData = res
    },
    // 更改员工岗位信息
    async saveJob() {
      await updateEmployeesJobs(this.Id, { ...this.formData })
    }
  }
}
</script>
