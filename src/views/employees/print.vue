    ...
    <div class="app-container">
      <el-card id="print">
        <el-row type="flex" justify="end">
          <el-button v-print="'#print'" size="small" type="primary">打印</el-button>
        </el-row>
        ...
        <div v-if="type === 'personal'">
          ...
          <table cellspacing="0" width="100%" class="tableList">
            ...
            <tr>
              <th>出生日期</th>
              <td colspan="6">{{ formData.dateOfBirth | formTime }}</td>
            </tr>
            <tr>
              <th>最高学历</th>
...
<script>
import { getEmployeesDetail, getEmployeesJobs } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getEmployeesDetail(this.userId) // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getInfoById(this.userId)
      const jobInfo = await getEmployeesJobs(this.userId) // 获取个人基本信息
